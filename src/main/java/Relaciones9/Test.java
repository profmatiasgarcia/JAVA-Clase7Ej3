...
public class Test {
    ...
    public static void main(String[] args) {
...

        {
            ...
            Query query = session.createSQLQuery("SELECT idMunicipio,barrio,localidad FROM municipio");
...
//            session.getTransaction().commit();
    ...
    public static void cargarProfesores() {
        ...
        session.save(lugar8);
        session.save(lugar9);
        session.save(lugar10);
        session.save(profesor);
        session.save(profesor2);
        session.save(profesor3);
        session.save(profesor4);
        session.save(profesor5);
        session.save(profesor6);
        session.save(profesor7);
        session.save(profesor8);
        session.save(profesor9);
        session.save(profesor10);
